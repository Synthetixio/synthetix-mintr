  ...
  const onDataLoaded = async recipientsData => {
    try {
      ...
      recipientsData.forEach(item => {
        ...
        recipientsShares.push(
          ethers.utils.parseEther(Number(item[1]).toFixed(6)).toString()
        );
      });

      const transactionData = airdropper.functions.multisend.encode([
        addresses.token,
        recipientsAddresses,
        recipientsShares,
      ]);
      const tx = await multisig.submitTransaction(
        addresses.airdropper,
        0,
        transactionData
      );
      if (tx) {
