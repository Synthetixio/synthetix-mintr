...
const SetAllowance = ({ createTransaction, goBack, currentGasPrice }) => {
	...
	const onUnlock = async () => {
		...
		try {
			...
			const gasEstimate = await curveSBTCContract.estimate.approve(
				sBTCRewardsContract.address,
				parseEther(TOKEN_ALLOWANCE_LIMIT.toString())
			);
			const transaction = await curveSBTCContract.approve(
				sBTCRewardsContract.address,
				parseEther(TOKEN_ALLOWANCE_LIMIT.toString()),
				{
			...
			if (transaction) {
				createTransaction({
					...
					status: 'pending',
					info: t(`curvepoolSBTC.locked.transaction`),
	...
	return (
		<>
			...
			<TitleContainer>
				<Logo src="/images/pools/iearn-sBTC.svg" />
				<PageTitle>{t('curvepoolSBTC.title')}</PageTitle>
				<PLarge>{t('curvepoolSBTC.locked.subtitle')}</PLarge>
			</TitleContainer>
			<ButtonRow>
