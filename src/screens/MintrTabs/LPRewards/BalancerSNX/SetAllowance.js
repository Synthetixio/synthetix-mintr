...
const SetAllowance = ({ createTransaction, goBack, currentGasPrice }) => {
	...
	const onUnlock = async () => {
		...
		try {
			...
			const gasEstimate = await balancerpoolContract.estimate.approve(
				balancerSNXRewardsContract.address,
				parseEther(TOKEN_ALLOWANCE_LIMIT.toString())
			);
			const transaction = await balancerpoolContract.approve(
				balancerSNXRewardsContract.address,
				parseEther(TOKEN_ALLOWANCE_LIMIT.toString()),
				{
			...
			if (transaction) {
				createTransaction({
					...
					status: 'pending',
					info: t('balancerSNX.locked.transaction'),
	...
	return (
		<>
			...
			<TitleContainer>
				<Logo src="/images/pools/balancer-SNX.svg" />
				<PageTitle>{t('balancerSNX.title')}</PageTitle>
				<PLarge>{t('balancerSNX.locked.subtitle')}</PLarge>
			</TitleContainer>
			<ButtonRow>
